...
int main(int argc, char **argv) {
    ...
    Synchronizer<autoware_detection_msgs::ImageObj, autoware_detection_msgs::PointsImage, autoware_detection_msgs::ImageObjRanged> synchronizer(sub1, sub2, pub1, pub2, req, ns);
    synchronizer.run();
...
boost::circular_buffer<autoware_detection_msgs::PointsImage> vscan_image_ringbuf(10);
...
autoware_detection_msgs::PointsImage* p_vscan_image_buf;

void publish_msg(autoware_detection_msgs::ImageObj* p_image_obj_buf, autoware_detection_msgs::PointsImage* p_vscan_image_buf) {
    ROS_INFO("publish");
...
bool publish() {
    if (buf_flag) {
        ...
        // image_obj < vscan_image
            ...
            boost::circular_buffer<autoware_detection_msgs::PointsImage>::iterator it = vscan_image_ringbuf.begin();
...
void vscan_image_callback(const autoware_detection_msgs::PointsImage::ConstPtr& vscan_image_msg) {
    pthread_mutex_lock(&mutex);
...
int main(int argc, char **argv) {
    ...
    vscan_image__pub = nh.advertise<autoware_detection_msgs::PointsImage>("/sync_ranging/vscan_image", 5);
...
autoware_detection_msgs::PointsImage vscan_image_buf;
