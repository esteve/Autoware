...
public:
  ...
  int findClosestCrosswalk(const int closest_waypoint, const autoware_detection_msgs::Lane &lane, const int search_distance);
  int getSize() const
  {
...
inline int calcWaypointIndexReverse(const autoware_detection_msgs::Lane &lane, const int begin_waypoint, const double distance)
{
  double dist_sum = 0;
