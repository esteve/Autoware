...
void DecisionMakerNode::insertPointWithinCrossRoad(const std::vector<CrossRoadArea> &_intersects,
                                                   autoware_detection_msgs::LaneArray &lane_array)
{
    ...
    for (auto &wp : lane.waypoints)
    {
      ...
      for (auto &area : intersects)
      {
        if (CrossRoadArea::isInsideArea(&area, pp))
        {
          ...
          {
            autoware_detection_msgs::Lane nlane;
	    ...
	    area.bbox.pose.orientation = wp.pose.pose.orientation;
          ...
          // area.insideLanes.Waypoints.push_back(wp);//autoware_detection_msgs::Waypoint
          // lane's wp
...
void DecisionMakerNode::setWaypointState(autoware_detection_msgs::LaneArray& lane_array)
{
  ...
  for (auto &area : intersects)
  {
    for (auto &laneinArea : area.insideLanes)
    {
      ...
      if (angle_deg <= ANGLE_LEFT)
        steering_state = autoware_detection_msgs::WaypointState::STR_LEFT;
      else if (angle_deg >= ANGLE_RIGHT)
        steering_state = autoware_detection_msgs::WaypointState::STR_RIGHT;
      else
        steering_state = autoware_detection_msgs::WaypointState::STR_STRAIGHT;
  ...
  std::vector<StopLine> stoplines = g_vmap.findByFilter([&](const StopLine& stopline) {
    return ((g_vmap.findByKey(Key<RoadSign>(stopline.signid)).type &
             (autoware_detection_msgs::WaypointState::TYPE_STOP | autoware_detection_msgs::WaypointState::TYPE_STOPLINE)) != 0);
  });
...
// for based waypoint
void DecisionMakerNode::callbackFromLaneWaypoint(const autoware_detection_msgs::LaneArray &msg)
{
  ...
  for (auto &lane : current_based_lane_array_.lanes)
  {
    ...
    for (auto &wp : lane.waypoints)
    {
      ...
      wp.lid = lid++;
      wp.wpstate.aid = 0;
      wp.wpstate.steering_state = autoware_detection_msgs::WaypointState::NULLSTATE;
      wp.wpstate.accel_state = autoware_detection_msgs::WaypointState::NULLSTATE;
      wp.wpstate.stopline_state = autoware_detection_msgs::WaypointState::NULLSTATE;
      wp.wpstate.lanechange_state = autoware_detection_msgs::WaypointState::NULLSTATE;
...
{
  if (msg_state == (uint8_t)autoware_detection_msgs::WaypointState::STR_LEFT)
    return state_machine::DRIVE_STR_LEFT_STATE;
  else if (msg_state == (uint8_t)autoware_detection_msgs::WaypointState::STR_RIGHT)
    return state_machine::DRIVE_STR_RIGHT_STATE;
  else
    return state_machine::DRIVE_STR_STRAIGHT_STATE;
}

void DecisionMakerNode::callbackFromFinalWaypoint(const autoware_detection_msgs::Lane &msg)
{
  if (!hasvMap())
  {
  ...
  for (size_t i = (previous_idx > idx) ? idx : previous_idx; i <= idx; i++)
  {
    ...
    {
      if (current_finalwaypoints_.waypoints.at(i).wpstate.stopline_state == autoware_detection_msgs::WaypointState::TYPE_STOPLINE)
      {
        ctx->setCurrentState(state_machine::DRIVE_ACC_STOPLINE_STATE);
        closest_stopline_waypoint_ = CurrentStoplineTarget_.gid;
      }
      if (current_finalwaypoints_.waypoints.at(i).wpstate.stopline_state == autoware_detection_msgs::WaypointState::TYPE_STOP)
