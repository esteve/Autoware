...
private:
  ...
  std::unordered_map<std::string, autoware_detection_msgs::Lane> waypoints_;
  std::unordered_map<std::string, ros::Publisher> Pubs;
  std::unordered_map<std::string, ros::Subscriber> Subs;

  autoware_detection_msgs::Lane final_waypoints_dp_;
  autoware_detection_msgs::Lane final_waypoints_astar_;
...
public:
  ...
  void callbackFromWaypoints(const ros::MessageEvent<autoware_detection_msgs::Lane const> &event);
