...
class Yolo2DetectorNode
{
	...
	void convert_rect_to_image_obj(std::vector< RectClassScore<float> >& in_objects, autoware_msgs::ImageObj& out_message, std::string in_class)
	{
	...
	void image_callback(const sensor_msgs::ImageConstPtr& in_image_message)
	{
		...
		autoware_msgs::ImageObj output_car_message;
		autoware_msgs::ImageObj output_person_message;
...
public:
	void Run()
	{
		...
		publisher_car_objects_ = node_handle_.advertise<autoware_msgs::ImageObj>("/obj_car/image_obj", 1);
		publisher_person_objects_ = node_handle_.advertise<autoware_msgs::ImageObj>("/obj_person/image_obj", 1);
